...
def mexico_filter(latitude,longitude):
    ...
    for response_dictionary in results:
        if (response_dictionary['name']=='Manhattan' and response_dictionary['cc']=='US')  or (response_dictionary['name']=='San Francisco' and response_dictionary['cc']=='US'):
            return True
        else:
            return False
...
for key_path in filtered_keys:
    ...
    for obj in file_list:
        if not str(obj['Key']).endswith("/"):
    	    ...
    	    output_path="s3a://"+target_s3_bucket+"/"+ key_path
            us_filter_df.write.format("com.databricks.spark.csv").option("codec","org.apache.hadoop.io.compress.GzipCodec").option("sep", "|").mode("append").save(output_path)
